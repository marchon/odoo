...
MODERATION_FIELDS = ['moderation', 'moderator_ids', 'moderation_email_ids', 'moderation_notify', 'moderation_notify_msg', 'moderation_guidelines', 'moderation_guidelines_msg']
...
class ModerationEmail(models.Model):
    """
        The purpose of this model is to handle the white & black lists of emails.
    """
    _name = 'mail.moderation.email'

    email = fields.Char(string="Email", index=True, required=True)
    status = fields.Selection([('allow', 'Always Allow'), ('ban', 'Permanent Ban')], string="Status", required=True)
...
class Channel(models.Model):
    ...
    moderation_email_ids = fields.One2many('mail.moderation.email', 'channel_id', string='Moderated Emails', groups="base.group_user")
    moderation_email_count = fields.Integer(string='Moderated emails count', compute='_compute_moderation_email_count', groups="base.group_user")
    ...
    def _compute_is_moderator(self):
        for channel in self:
            if self.env.user in channel.moderator_ids:
                channel.is_moderator = True
            else:
                channel.is_moderator = False

    @api.multi
    @api.depends('moderation_email_ids')
    def _compute_moderation_email_count(self):

        read_group_res = self.env['mail.moderation.email'].read_group([('channel_id', 'in', self.ids)], ['channel_id', 'email', 'status'], 'channel_id')

        # Note: dictionary['channel_id'] is a pair (channel.id,channel.name) for some channel in self.
        result = {}
        for dictionary in read_group_res:
            result[dictionary['channel_id'][0]] = dictionary['channel_id_count']

        for channel in self:
            channel.moderation_email_count = result.get(channel.id, 0)
    ...
    def send_guidelines(self):
        ...
        if self.env.user in self.moderator_ids or self.env.user.has_group('base.group_system'):
            ...
            if not success:
                raise UserError('The template "mail.mail_template_guidelines_notification_email" was not found. No email has been sent. Please contact an administrator to fix this issue.')
        else:
    ...
    def _send_guidelines(self, partners):
        # This method is called by a cron once a day. This explain the form of the code below. We don't want to raise an error
        # if the template does not exist.
        self.ensure_one()
        template = self.env.ref('mail.mail_template_guidelines_notification_email', raise_if_not_found=False)
        if template:
            banned_emails = self.env['mail.moderation.email'].search([
                    ('status', '=', 'ban'),
                    ('channel_id', 'in', self.ids)
                ]).mapped('email')
            good_partners = partners.filtered(lambda p: p.email and not (p.email in banned_emails))
            for partner in good_partners:
                '''the sudo is needed because because send_mail will create a message (a mail). Since in the template, 
                email_from is linked to the company and not to the moderator, _check_acces_rule will check if the
                moderator can write on the model res_partner because that model is the model associated with the message.
                '''
                template.with_context(lang=partner.lang, channel=self).sudo().send_mail(partner.id)
            return True
        else:
            _logger.warning('The template "mail.mail_template_guidelines_notification_email" was not found.')
            return False
    ...
    def _update_moderation_email(self, emails, status):
        ...
        moderated = self.env['mail.moderation.email'].search([
                ('email', 'in', emails),
                ('channel_id', 'in', self.ids)
            ])
        not_moderated = [email for email in emails if email not in moderated.mapped('email')]
        cmds = []
        for record in moderated:
                cmds.append((1, record.id, {'status': status}))
        cmds = cmds + [(0, 0, {'email': email, 'status': status}) for email in not_moderated]
    ...
    def write(self, vals):
        ...
        touched = any(key for key in MODERATION_FIELDS if vals.get(key))
        if touched and any(self.env.user not in channel.moderator_ids for channel in self if channel.moderation):
        ...
        if vals.get('moderation') is False:
            messages_to_accept = self.env['mail.message'].search(
                        [
            ...
            messages_to_accept._accept_and_notify()
    ...
    def _subscribe_users(self):
        for mail_channel in self:
            partner_ids = mail_channel.mapped('group_ids.users.partner_id').ids
            mail_channel.write({'channel_partner_ids': [(4, pid) for pid in partner_ids]})
    ...
    def _extract_values(self, message_type, **kwargs):
        ...
        if self.moderation and message_type in ['email', 'comment']:
            ...
            status = self.env['mail.moderation.email'].sudo().search([('email', '=', email), ('channel_id', 'in', self.ids)]).mapped('status')
