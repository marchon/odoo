...
class StockMove(models.Model):
    ...
    def _prepare_account_move_line(self, qty, cost, credit_account_id, debit_account_id): #TODO OCO c'est ici dedans qu'il va falloir bidouiller, je pense !
        ...
        if self._is_out() and self.company_id.anglo_saxon_accounting and self._get_related_invoices():
            ...
            if interim_output_invoice_move_lines:
                res += interim_output_invoice_move_lines.get_aml_data_to_fully_reconcile_out_invoices_with_stock_valuation(interim_output_account, self.product_id, self._get_partner_id_for_valuation_lines(), previously_shipped_qty, self.product_qty, debit_val)

            # TODO OCO remplacer par l'appel à ta méthode magique, quand tu l'auras
            """
            ...
            difference_with_invoice = self.company_id.currency_id.round((debit_val / self.product_qty) * total_treated_qty + invoice_valuation)
    ...
    def _get_related_invoices(self):
        ...
        rslt += self.mapped('picking_id.sale_id.invoice_ids').filtered(lambda x: x.state not in ('draft', 'cancel'))
        return rslt
