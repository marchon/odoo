...
class TestCreatePicking(common.TestProductCommon):
    ...
    def test_03_mto_multiple_po(self):
        ...
        merged_customer_move = self.env['stock.move'].create({
            'name': 'move out',
            'location_id': stock_location.id,
            'location_dest_id': customer_location.id,
            'product_id': product.id,
        ...
        merged_customer_move._action_confirm()

        self.assertFalse(merged_customer_move.exists(), 'The second customer move should be merged in the first.')
        self.assertEqual(customer_move.product_uom_qty, 100.0, 'The quantity should be the sum of the two moves.')
        ...
        self.assertEqual(customer_move.reserved_availability, 100.0, 'The total quantity for the customer move should be available and reserved.')
