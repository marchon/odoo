...
class PaymentTxOgone(models.Model):
    ...
    def _ogone_form_validate(self, data):
        ...
        if status in self._ogone_valid_tx_status:
            vals = {
                'payment_date': datetime.datetime.strptime(data['TRXDATE'], '%m/%d/%y').strftime(DEFAULT_SERVER_DATE_FORMAT),
    ...
    def _ogone_s2s_validate_tree(self, tree, tries=2):
        ...
        if status in self._ogone_valid_tx_status:
            self.write({
                'payment_date': datetime.date.today().strftime(DEFAULT_SERVER_DATE_FORMAT),
