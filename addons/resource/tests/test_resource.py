...
class TestTimeType(TestResourceCommon):
    ...
    def test_default_time_type(self):
        self.assertEqual(self.leave1.time_type, 'leave')
        self.assertEqual(self.leave2.time_type, 'leave')
        self.assertEqual(self.other1.time_type, 'other')

    def test_leaves_default(self):
        # Here we test if the default value is correctly set to leave and used
        leave = self.calendar._get_day_leave_intervals(Date.from_string('2013-02-26'), time(0), time(23, 59, 59), self.resource1_id)
        self.assertEqual(len(leave), 1, "By default resource should compute the leaves")
        leave = self.calendar._get_day_leave_intervals(Date.from_string('2013-02-15'), time(0), time(23, 59, 59), self.resource1_id)
        self.assertEqual(len(leave), 0, "By default resource should only compute the leaves")

    def test_leaves_leaves(self):
        # Here we test if setting the value to leave has the effect intended
        leave = self.calendar._get_day_leave_intervals(Date.from_string('2013-02-26'), time(0), time(23, 59, 59), self.resource1_id, 'leave')
        self.assertEqual(len(leave), 1, "When asking for leave should compute the leaves")
        leave = self.calendar._get_day_leave_intervals(Date.from_string('2013-02-15'), time(0), time(23, 59, 59), self.resource1_id, 'leave')
    ...
    def test_leaves_other(self):
        ...
        leave = self.calendar._get_day_leave_intervals(Date.from_string('2013-02-26'), time(0), time(23, 59, 59), self.resource1_id, 'other')
        self.assertEqual(len(leave), 0, "When asking for other should ignore leaves")
        leave = self.calendar._get_day_leave_intervals(Date.from_string('2013-02-15'), time(0), time(23, 59, 59), self.resource1_id, 'other')
