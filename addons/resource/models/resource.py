...
class ResourceCalendar(models.Model):
    ...
    @api.multi
    def _get_leave_intervals(self, resource_id=None, start_datetime=None, end_datetime=None, time_type='leave'):
    ...
    @api.multi
    def _get_day_work_intervals(self, day_date, start_time=None, end_time=None, compute_leaves=False, resource_id=None, time_type='leave'):
    ...
    def _get_day_leave_intervals(self, day_date, start_time, end_time, resource_id, time_type='leave'):
    ...
    def _iter_work_intervals(self, start_dt, end_dt, resource_id, compute_leaves=True, time_type='leave'):
    ...
    def _iter_leave_intervals(self, start_dt, end_dt, resource_id, time_type='leave'):
    ...
    def _iter_work_hours_count(self, from_datetime, to_datetime, resource_id, time_type='leave'):
        """ Lists the current resource's work hours count between the two provided
        datetime expressed in naive UTC. """

        for interval in self._iter_work_intervals(from_datetime, to_datetime, resource_id, time_type):
            td = timedelta()
            for work_interval in interval:
                td += work_interval[1] - work_interval[0]
            yield (interval[0][0].date(), td.total_seconds() / 3600.0)

    def _iter_work_days(self, from_date, to_date, resource_id, time_type='leave'):
    ...
    @api.multi
    def get_work_hours_count(self, start_dt, end_dt, resource_id, compute_leaves=True, time_type='leave'):
        """ Count number of work hours between two datetimes. For compute_leaves,
        resource_id: see _get_day_work_intervals. """
        res = timedelta()
        for intervals in self._iter_work_intervals(start_dt, end_dt, resource_id, compute_leaves=compute_leaves, time_type=time_type):
            for interval in intervals:
                res += interval[1] - interval[0]
    ...
    @api.multi
    def _schedule_hours(self, hours, day_dt, compute_leaves=False, resource_id=None, time_type='leave'):
    ...
    @api.multi
    def plan_hours(self, hours, day_dt, compute_leaves=False, resource_id=None, time_type='leave'):
        """ Return datetime after having planned hours """
        res = self._schedule_hours(hours, day_dt, compute_leaves, resource_id, time_type)
    ...
    @api.multi
    def _schedule_days(self, days, day_dt, compute_leaves=False, resource_id=None, time_type='leave'):
        ...
        while planned_days < abs(days) and iterations < 100:
            working_intervals = self._get_day_work_intervals(
                ...
                compute_leaves=compute_leaves, resource_id=resource_id, time_type=time_type)
    ...
    @api.multi
    def plan_days(self, days, day_dt, compute_leaves=False, resource_id=None, time_type='leave'):
        """ Returns the datetime of a days scheduling. """
        res = self._schedule_days(days, day_dt, compute_leaves, resource_id, time_type)
