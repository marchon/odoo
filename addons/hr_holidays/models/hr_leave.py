...
class HolidaysRequest(models.Model):
    ...
    def _calc_days_temp(self):
        ...
        if is_set and is_set_date:
            ...
            if self.unit == 'day' or (self.unit == 'period' and self.method == 'day'):
                hour_from = attendance_from.hour_from
                hour_to = attendance_to.hour_to

                date_from = fields.Datetime.to_string(datetime.combine(first_day, self._get_time_from_hour(hour_from, (-1, 0, 0))))
                date_to = fields.Datetime.to_string(datetime.combine(first_day if self.unit == 'day' else last_day, self._get_time_from_hour(hour_to)))
            elif self.unit == 'half' or (self.unit == 'period' and self.method == 'half'):
    ...
    def create(self, values):
        ...
        if values.get('unit', 'period') != 'period':
