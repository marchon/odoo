...
class TestHolidaysFlow(TestHrHolidaysBase):
    ...
    def test_00_leave_request_flow(self):
        ...
        # HrManager validates the employee leave request -> should not work
        with self.assertRaises(UserError):
            hol12_manager_group.action_approve()
        self.assertEqual(hol12_user_group.state, 'confirm', 'hr_holidays: hr manager should not be able to validate department manager only leaves')
        ...
        # HrManager validates the first step -> crash should not be able to
        with self.assertRaises(UserError):
            aloc1_user_group.sudo(self.user_hrmanager_id).action_approve()
        ...
        # HrManager validates the first step -> should crash
        with self.assertRaises(UserError):
