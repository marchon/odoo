...
class TestSaleToInvoice(TestCommonSaleNoChart):
    ...
    def setUpClass(cls):
        ...
        # create SO with 4 products
        order_form = Form(cls.env['sale.order'])
        order_form.partner_id = cls.partner_customer_usd
        with order_form.order_line.new() as line:
            line.product_id = cls.product_order
        with order_form.order_line.new() as line:
            line.product_id = cls.service_deliver
        with order_form.order_line.new() as line:
            line.product_id = cls.service_order
        with order_form.order_line.new() as line:
            line.product_id = cls.product_deliver
    ...
    def test_downpayment(self):
        ...
        payment_form = Form(self.env['sale.advance.payment.inv'].with_context(context))
        payment_form.advance_payment_method = 'fixed'
        payment_form.amount = 100
        payment = payment_form.save()
        invoice = payment.create_invoices()
