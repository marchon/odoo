...
class PaymentTransaction(models.Model):
    ...
    def _wepay_form_get_invalid_parameters(self, data):
        ...
        # check what is buyed
        if float_compare(float(data.get('amount', '0.0') + data.get('fee').get('app_fee')), (self.amount + self.fees), 2) != 0:
            invalid_parameters.append(('amount', data.get('amount'), '%.2f' % self.amount))  # mc_gross is amount + fees
        if data.get('currency') != self.currency_id.name:
            invalid_parameters.append(('currency', data.get('currency'), self.currency_id.name))
        if 'app_fee' in data['fee'] and float_compare(float(data.get('fee').get('app_fee')), self.fees, 2) != 0:
