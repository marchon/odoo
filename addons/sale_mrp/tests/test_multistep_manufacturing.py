...
class TestMultistepManufacturing(TestMrpCommon):
    ...
    def setUp(self):
        ...
        self.product_manu = Product.create({
            'name': 'Stick',
            'uom_id': self.uom_unit.id,
            'uom_po_id': self.uom_unit.id})
        # Create raw product for manufactured product
        self.product_raw = Product.create({
            'name': 'raw Stick',
            'uom_id': self.uom_unit.id,
            'uom_po_id': self.uom_unit.id})
        # Change routes of manufactured product
    ...
    def test_00_manufacturing_step_one(self):
        ...
        mo_procurement = self.MrpProduction.search([('procurement_group_id', '=', self.sale_order.procurement_group_id.id)])
