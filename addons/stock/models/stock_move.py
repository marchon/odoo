            ...
            for move in self:
                if move.move_dest_ids and move.propagate:
                    ...
                    if propagated_date_field:
                        ...
                        new_date = datetime.from_string(vals.get(propagated_date_field))
                        delta = relativedelta(new_date, current_date)
                        if abs(delta.days) >= move.company_id.propagation_minimum_delta:
                            old_move_date = move.move_dest_ids[0].date_expected
                            new_move_date = (old_move_date + relativedelta(days=delta.days or 0))
