...
class SaleOrderLine(models.Model):
    ...
    project_id = fields.Many2one('project.project', 'Generated Project', copy=False, help="Project generated by the sales order item")
    task_id = fields.Many2one('project.task', 'Generated Task', copy=False, help="Task generated by the sales order item")
    ...
    def create(self, values):
        ...
        if line.state == 'sale' and not line.is_expense:
            line._timesheet_service_generation(message_on_sale_order=True)
        return line
    ...
    @api.multi
    def _timesheet_create_task(self, project, message_on_sale_order=False):
        """ Generate task for the given so line, and link it.
            :param project: record of project.project in which the task should be created
            :param message_on_sale_order: True if a message should be post on SO when task is created
            :return task: record of the created task
        """
    ...
    @api.multi
    def _timesheet_service_generation(self, message_on_sale_order=False):
            ...
            for so_line in so_line_task_global_project:
                if not so_line.task_id:
                    if map_sol_project.get(so_line.id):
                        so_line._timesheet_create_task(project=map_sol_project[so_line.id], message_on_sale_order=message_on_sale_order)
        ...
        if so_line_new_project:
            for so_line in so_line_new_project:
                ...
                if so_line.product_id.service_tracking == 'task_new_project':
                    ...
                    so_line._timesheet_create_task(project=project, message_on_sale_order=message_on_sale_order)
