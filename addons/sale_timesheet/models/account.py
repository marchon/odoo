...
class AccountAnalyticLine(models.Model):
    ...
    def _timesheet_postprocess_values(self, values):
        ...
        if any([field_name in values for field_name in ['so_line', 'unit_amount', 'account_id']]):
            values_to_write = sudo_self._timesheet_compute_theorical_revenue()
            for timesheet_id, vals in values_to_write.items():
                if vals:
                    result[timesheet_id].update(vals)
        return result
